...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Edit Penebaran Benih', href: '/kolam-seeding' }, // tidak perlu url create
];
...
export default function Update({ kolam_options, user_role, user_kolam_ids, feeding }: Props) {
    ...
    useEffect(() => {
        const kolam = filteredKolamOptions.find(k => k.value === feeding.kolam_id) || null;
        setSelectedKolam(kolam);
    }, [feeding.kolam_id, filteredKolamOptions]);

    const form = useForm({
        id: feeding.id,
        kolam_id: feeding.kolam_id,
        feeding_date: feeding.feeding_date,
        feed_type: feeding.feed_type,
        quantity_kg: feeding.quantity_kg,
        feeding_method: feeding.feeding_method,
        feed_source: feeding.feed_source,
        notes: feeding.notes || '',
    });
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        form.put(`/kolam-feeding/${feeding.id}`, {
            onSuccess: () => router.get('/kolam-feeding'),
        });
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Update Penebaran Benih" />
            ...
            <div className="max-w-full mx-auto p-4">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white dark:bg-neutral-900 rounded-md shadow-sm p-6">
                        <h2 className="text-xl font-semibold mb-4">Form Penebaran Benih</h2>
                        ...
                        <form onSubmit={submit} className="space-y-4">
                                ...
                                {selectedKolam?.seeding?.length ? (
                                    <div className="mt-3 space-y-3">
                                        {selectedKolam.seeding.map((s: any) => (
                                            <div
                                                key={s.id}
                                                className="p-4 border rounded-lg shadow-sm bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-700"
                                            >
                                                {/* Badge */}
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-neutral-700 dark:text-neutral-300">
                                                    <div className="flex justify-between"><span className="font-medium">Ukuran (cm)</span><span>{s.seed_size_cm ?? '-'}</span></div>
                                                    <div className="flex justify-between"><span className="font-medium">Berat Rata-rata (g)</span><span>{s.average_weight_seed_g ?? '-'}</span></div>
                                                    <div className="flex justify-between"><span className="font-medium">Tanggal Tebar</span><span>{s.date_seeded.split('T')[0]}</span></div>
                                                    <div className="flex justify-between"><span className="font-medium">Sumber</span><span>{s.source ?? '-'}</span></div>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <FormInput
                                    label="Tanggal Sebar Benih"
                                    name="feeding_date"
                                    type="date"
                                    value={form.data.feeding_date}
                                    onChange={(e) => form.setData('feeding_date', e)}
                                    error={form.errors.feeding_date}
                                />
                                <FormInput
                                    label="Jenis Benih"
                                    name="seed_type"
                                    type="text"
                                    value={form.data.feed_type}
                                    onChange={(e) => form.setData('feed_type', e)}
                                    error={form.errors.feed_type}
                                />
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <FormInput
                                    ...
                                    name="quantity_kg"
                                    type="number"
                                    value={form.data.quantity_kg}
                                    onChange={(e) => form.setData('quantity_kg', e)}
                                    error={form.errors.quantity_kg}
                                />
                                <FormInput
                                    label="Methode Pemberian Pakan"
                                    name="feeding_method"
                                    type="text"
                                    value={form.data.feeding_method}
                                    onChange={(e) => form.setData('feeding_method', e)}
                                    error={form.errors.feeding_method}
                                />
                            ...
                            <FormInput
                                ...
                                name="feed_source"
                                type="text"
                                value={form.data.feed_source}
                                onChange={(e) => form.setData('feed_source', e)}
                                error={form.errors.feed_source}
                            />
                            {/* Remarks */}
                            <FormInput
                                label="Catatan"
